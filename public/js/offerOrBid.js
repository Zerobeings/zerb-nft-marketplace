...
if(makeOffer != null){
    getit.addEventListener("click", async () => { //add connection to creat listing button
        ...
        //mainnet Weth
        let mWETH = await fetch("/json/weth.json").then((r) => {
            return r.json()
        })
        ...
        try {
                ...
                if (net != 1) {
                    ...
                    if(buyitnow != null && buyListing !=null && buyAtAmount !=null && net === 1){
                    ...
                    makeOffer.addEventListener("submit", async (e) => { 
                        ...
                        if(listingType === "0" && net === 1) {
                            try {
                                const m_WETH_contract = new web3.eth.Contract(mWETH, WETH);
                                //check if wallet has laready approved the spend limit
                                var checkSpendLimit = await m_WETH_contract.methods.allowance(`${selectedAccount}`, marketplaceContract).call({from:`${selectedAccount}`});
                                const wethBalance = await m_WETH_contract.methods.balanceOf(`${selectedAccount}`).call({from:`${selectedAccount}`});
                                ...
                                if(checkSpendLimit >= pricePerToken && wethBalance >= pricePerToken){
                                    ...
                                    const etimateGasApprove = await m_WETH_contract.methods.approve(marketplaceContract, pricePerToken).estimateGas({from: `${selectedAccount}`});
                                    var etimate_GasApprove = Math.round(etimateGasApprove * 1.2); // estimatation based on transaction

                                    const txApprove = await m_WETH_contract.methods.approve(marketplaceContract, pricePerToken).send({from:`${selectedAccount}`, gas:web3.utils.toHex(etimate_GasApprove), gasPrice:web3.utils.toHex(gas_price)});
