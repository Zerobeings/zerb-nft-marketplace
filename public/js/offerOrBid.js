                ...
                makeOffer.addEventListener("submit", async (e) => { 
                    ...
                    if(listingType === "0") {
                        try {
                            ...
                            const checkSpendLimit = await m_WETH_contract.methods.allowance(`${selectedAccount}`, marketplaceContract).call({from:`${selectedAccount}`});
                            ...
                            if(checkSpendLimit >= pricePerToken){
                                ...
                                const txApprove = await m_WETH_contract.methods.approve(marketplaceContract, pricePerToken).send({from:`${selectedAccount}`});
